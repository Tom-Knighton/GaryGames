...
class MainPageGamesCell: UICollectionViewCell {
    
    @IBOutlet weak var GameLabel: UILabel?
    @IBOutlet weak var GameImage: UIImageView?
    @IBOutlet weak var HolderCell: UIView?
    
    
    func populate(with game: GameType) {
        self.gradientBackgroundColor()
        self.GameLabel?.text = game.GameName ?? ""
        self.GameImage?.sd_setImage(with: URL(string: game.GameImageURL ?? ""), completed: nil)
    ...
    func gradientBackgroundColor() {
        ...
        self.HolderCell?.setGradientBackgroundColor(colorOne: colors[0], colorTow: colors[1])
    }
}
